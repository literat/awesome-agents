...
/**
 * Create nominal/branded types to prevent primitive obsession at the type level.
 *
 * Note: Branded types provide compile-time type safety only. They do not provide
 * runtime validation or prevent runtime type confusion.
 *
 * @example
 * type UserId = Brand<string, 'UserId'>
 * type OrderId = Brand<string, 'OrderId'>
 *
 * // Compile-time error: Type 'OrderId' is not assignable to 'UserId'
 ...
 *
 * Note: Due to TypeScript's recursion depth limit (~50), this will cause
 * compilation errors for very large tuple sizes. For large tuples, use T[] instead.
 *
 * @example
 * type Point3D = Tuple<number, 3> // [number, number, number]
 */
